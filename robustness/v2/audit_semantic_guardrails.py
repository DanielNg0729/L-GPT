...
from robustness.v2.semantic_guardrails import AttributePrediction, FamilyPrediction, GuardrailConfig, evaluate as guard, load_dictionary
from robustness.v2.semantic_evidence_tier import SemanticEvidence, SemanticIntegrationPolicy, apply_policy
from submission.agent import Agent, CONSTRAINT

ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "robustness" / "v2" / "results" / "semantic_guardrail_audit.json"
...
def main() -> None:
    ...
    result = {
        ...
        "legacy_semantic_shift": {
            "development200": run_suite(ROOT / "robustness" / "v2" / "sets" / "semantic_attribute_development_200.jsonl", ids, categories, products, catalog),
            "holdout800": run_suite(ROOT / "robustness" / "v2" / "sets" / "semantic_attribute_holdout_800.jsonl", ids, categories, products, catalog),
        },
    }
    if not args.legacy_only:
        result["canonical_traffic"] = {
            "official200": run_suite(ROOT / "robustness" / "v2" / "public_value_only" / "official200_canonical_replay.jsonl", ids, categories, products, catalog),
