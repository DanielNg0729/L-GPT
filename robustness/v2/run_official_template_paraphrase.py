...
spec=importlib.util.spec_from_file_location('stress',ROOT/'experiments/scripts/31_paraphrase_stress.py');stress=importlib.util.module_from_spec(spec);spec.loader.exec_module(stress)
WRAP=ROOT/'robustness/v2/v1_turn_gated_bank/final_test.jsonl'
# Retain each V1 clarification-policy comparison as a distinct reproducible artifact.
OUT=ROOT/'robustness/v2/results/template_paraphrase9600_official200_wrapper_test_v1_information_gain.json'
def bank():
 d=defaultdict(list)
