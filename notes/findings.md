...
Reproduce: `PYTHONIOENCODING=utf-8 python -u notes/eda/0N_*.py` (passes 01-08).

---

## 0.5 Headline result **[OFFICIAL]**
...
| MRR | 0.0680 | **0.7761** | 1.000 |
| MTTC | 9.81 | **1.865** | 1.390 |
| Efficiency | 0.119 | **0.9135** | 0.9610 |
| **TechnicalScore** | **0.1067** | **0.91052** | **0.9922** |

**8.53× the published baseline; 91.8% of the achievable maximum** (the ceiling is 0.9922,
not 1.0 — `intent_override` gating puts a floor of 1.39 under MTTC). No LLM, no embeddings,
no network, Python standard library only.

| Scenario | n | HR@10 | MRR | MTTC |
|---|---|---|---|---|
| boundary | 10 | 100% | 0.831 | 2.10 |
| browsing | 80 | 98.8% | 0.733 | 1.69 |
| buying | 80 | 98.8% | 0.758 | 1.36 |
| intent_override | 30 | 100% | 0.921 | **3.60 = structural floor** |

`intent_override` reaches MTTC **3.60**, which *is* the floor for that scenario (the harness
gates hits until the override fires at turn 3 or 4, mean 3.60). Those sessions are solved as
early as the rules permit, and are now the system's *best*-ranked scenario — the override
message supplies a second verbatim constraint for free.
...
| Threat | Assessment |
|---|---|
| **Overfitting to the public 200** | Real. Tuning is done by coordinate ascent on the public set; the private set has different users *and* different target products. Mitigation: prefer flat regions of the sweep over sharp peaks, and report fold variance (§eda/07). |
...
Full writeup: [`robustness_audit.md`](robustness_audit.md). Summary of what is new.
...
50,000 products). Full writeup: `ml_lit_review.md` section F.
...
Source: [`competition_slides_verbatim.md`](competition_slides_verbatim.md), plus the live
