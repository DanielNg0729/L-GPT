"""EDA pass 48: coarse global hyper-parameter search (Optuna), parked in a dashboard-readable DB.
...
STORAGE. SQLite at notes/eda/optuna_coarse.db, so the study is inspectable live:

    optuna-dashboard sqlite:///notes/eda/optuna_coarse.db

and resumable -- re-running this script continues the same study rather than starting over.

Run:  PYTHONIOENCODING=utf-8 python -u notes/eda/48_optuna_coarse.py --hours 3 --workers 6
"""
...
DB = ROOT / "notes" / "eda" / "optuna_coarse.db"
...
def suggest(trial) -> dict:
    """~20 knobs. Ranges from notes/hyperparams.md; wide, because this pass is a map."""
    return {
        # --- ranking weights
