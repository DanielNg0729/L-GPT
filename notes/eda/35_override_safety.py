"""EDA pass 35: the one failure mode that is CATASTROPHIC rather than merely degrading.
...
Run:  PYTHONIOENCODING=utf-8 python -u notes/eda/35_override_safety.py
"""
...
def main() -> None:
    ...
    (ROOT / "notes" / "eda" / "out_35.json").write_text(
        json.dumps({f"{k[0]} | {k[1]}": v for k, v in OUT.items()}, indent=2) + "\n",
        encoding="utf-8")
    print("\n[saved] notes/eda/out_35.json")
